...
# --- PAIEMENTS & PLAN PREMIUM (Stripe) ---
# Facultatif : sans ces variables, la page /upgrade affiche un message "bientôt disponible".
# Les limites du plan gratuit (ci-dessous) s'appliquent dans tous les cas, pour que le modèle
# économique soit prêt dès que Stripe est branché, sans devoir toucher au code plus tard.
STRIPE_SECRET_KEY = os.environ.get("STRIPE_SECRET_KEY", "")
STRIPE_PUBLISHABLE_KEY = os.environ.get("STRIPE_PUBLISHABLE_KEY", "")
STRIPE_PRICE_ID = os.environ.get("STRIPE_PRICE_ID", "")
STRIPE_WEBHOOK_SECRET = os.environ.get("STRIPE_WEBHOOK_SECRET", "")
STRIPE_ENABLED = bool(STRIPE_LIB_AVAILABLE and STRIPE_SECRET_KEY and STRIPE_PRICE_ID)
if STRIPE_ENABLED:
    stripe.api_key = STRIPE_SECRET_KEY
elif STRIPE_SECRET_KEY and not STRIPE_LIB_AVAILABLE:
    log.warning("STRIPE_SECRET_KEY défini mais le paquet stripe n'est pas installé : paiements désactivés.")

# Limites du plan gratuit — s'appliquent toujours, Stripe configuré ou non.
FREE_LIST_LIMIT = 3
FREE_SHARE_LIMIT_PER_LIST = 1
...
def user_is_premium(uid):
    conn = get_db()
    row = conn.execute("SELECT is_premium, premium_until FROM users WHERE id=?", (uid,)).fetchone()
    ...
    if row[1]:
        try:
            ...
            return True
    return True  # abonnement payant actif, géré par le webhook Stripe (pas de date de fin locale)


def grant_premium_days(conn, uid, days):
    """Ajoute des jours de Premium (programme de parrainage), cumulables avec un abonnement en cours."""
    ...
    conn.execute("UPDATE users SET is_premium=1, premium_until=? WHERE id=?", (new_expiry.isoformat(), uid))


def generate_unique_referral_code(conn):
    while True:
        code = secrets.token_hex(4)
        if not conn.execute("SELECT id FROM users WHERE referral_code=?", (code,)).fetchone():
            return code
...
def send_email(to_addr, subject, body):
    """Envoie un e-mail simple. No-op silencieux (avec log) si le SMTP n'est pas configuré."""
...
def migrate_db():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        for (uid,) in conn.execute("SELECT id FROM users WHERE referral_code IS NULL").fetchall():
            conn.execute("UPDATE users SET referral_code=? WHERE id=?", (generate_unique_referral_code(conn), uid))
        conn.commit()
...
def get_visible_lists(uid):
    """Retourne (mes_listes, listes_partagees_avec_moi)."""
    conn = get_db()
...
def share_list():
    ...
    if not target:
        ...
        conn.execute("""
            ...
            ON CONFLICT(owner_id, shared_with_id) DO UPDATE SET list_id=excluded.list_id
