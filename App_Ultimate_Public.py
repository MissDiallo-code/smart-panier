...
<body>
    <div class="hero">
        ...
        <p class="lead text-secondary mb-4 feat">Gérez votre budget courses intelligemment, créez plusieurs listes (courses, ménage, anniversaire...), partagez-les avec vos proches et recevez de vraies alertes, même app fermée.</p>
...
def register():
    if request.method == "POST":
        ...
        if not u:
            ...
            try:
                with sqlite3.connect(DB_NAME) as conn:
                    cur = conn.execute("INSERT INTO users (username, password) VALUES (?,?)", (u, generate_password_hash(p)))
...
def profile():
    ...
    return render_template_string(PROFILE_HTML, username=session["user"], shared_users=shared_users, my_lists=my_lists)
...
def api_del(item_id):
    ...
    if cur.rowcount == 0:
        return jsonify({"ok": False, "error": "Article introuvable."}), 404
    conn.commit()
    return jsonify({"ok": True, "total": _current_total(conn, list_id)})
